...
class LLMJudgeEvaluator(BaseEvaluator):
    def __init__(self, judge_model: str = "openai/gpt-4o-mini", api_key: str = None):
...
{{
  ...
  "reasoning": "one brief sentence summarizing the evaluation"
        ...
        try:
            response = await self.client.chat.completions.create(model=self.judge_model, messages=[{"role": "user", "content": prompt}], temperature=0.0)
            raw_output = response.choices[0].message.content
            cleaned = raw_output.strip().removeprefix("```json").removeprefix("```").removesuffix("```").strip()
            verdict = json.loads(cleaned)
            claims = verdict.get("claims", [])
            if not claims:
                result = {"score": -1.0, "explanation": "Judge extracted no claims.", "evidence_breakdown": {"claims": []}, "status": "indeterminate"}
                await set_cached(cache_key, result)
                return result
            supported = sum(1 for c in claims if c.get("status") == "supported")
            score = supported / len(claims)
            result = {"score": score, "explanation": verdict.get("reasoning", ""), "evidence_breakdown": {"claims": claims}, "status": "success"}
            await set_cached(cache_key, result)
            return result
        except Exception as e:
            return {"score": -1.0, "explanation": f"Judge API error: {str(e)}", "evidence_breakdown": {}, "status": "evaluator_error"}
